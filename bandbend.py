...
def _bend_function(epsilon: float, phi: float, nd: float, n_as: float, e_as: float,
                   e_f: float, t: float, e_out: float) -> float:
    """
    ...
    e, eV = 1, 1
    n_as = n_as * (1. / (1. + np.exp((e_as + phi - e_f) * eV / (k * 6.24e11 * t)))) + e_out * 3.3 * 1e-5 / (4 * np.pi * e)
    w = (epsilon * phi * eV * nd / (2 * np.pi * e ** 2)) ** 0.5
    return w - n_as


def _diff_function(epsilon: float, phi: float, Nd: float, Nas: float, Eas: float,
...
def _dichotomy_method(epsilon: float, phi0: float, phi1: float, nd: float, n_as: float, e_as: float, e_f: float,
    ...
    """
    counter += 1
    f_a = _bend_function(epsilon=epsilon, phi=phi0, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out)
    phi_i = (phi0 + phi1) / 2
    f_i = _bend_function(epsilon=epsilon, phi=phi_i, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out)
...
def _fixed_point_method(epsilon: float, phi: float, phi_fixed: float, nd: float, n_as: float, e_as: float, e_f: float, t: float,
    ...
    if np.abs(_bend_function(epsilon=epsilon, phi=phi, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out)) <= tolerance:
        return phi, count
    else:
        diff = _diff_function(epsilon=epsilon, phi=phi_fixed, Nd=nd, Nas=n_as, Eas=e_as, Ef=e_f, t=t)
        _lambda = 1 / diff
        phi_i = -_lambda * _bend_function(epsilon=epsilon, phi=phi, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out)
...
def _newtown_method(epsilon: float, phi: float, nd: float, n_as: float, e_as: float, e_f: float, t: float,
    ...
    if np.abs(_bend_function(epsilon=epsilon, phi=phi, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out)) <= tolerance:
        ...
        _lambda = -_bend_function(epsilon=epsilon, phi=phi, nd=nd, n_as=n_as, e_as=e_as, e_f=e_f, t=t, e_out=e_out) / \
                  _diff_function(epsilon=epsilon, phi=phi, Nd=nd, Nas=n_as, Eas=e_as, Ef=e_f, t=t)
...
def _secant_method(epsilon: float, phi_0: float, nd: float, n_as: float, e_as: float, e_f: float, t: float,
    ...
    if phi_1 is not None:
        if (_bend_function(epsilon=epsilon, phi=phi_1, nd=nd, n_as=n_as, e_as=e_as,
                           e_f=e_f, t=t, e_out=e_out)) < tolerance:
            return phi_1, counter
        else:
            factor = delta / (_bend_function(epsilon=epsilon, phi=phi_1,
                                             nd=nd, n_as=n_as, e_as=e_as,
                                             e_f=e_f, t=t, e_out=e_out) - _bend_function(epsilon=epsilon, phi=phi_0,
                                                                                         nd=nd, n_as=n_as, e_as=e_as,
                                                                                         e_f=e_f, t=t, e_out=e_out))
            phi_ = phi_1 - _bend_function(epsilon=epsilon,
                                          phi=phi_1, nd=nd,
                                          n_as=n_as, e_as=e_as,
                                          e_f=e_f, t=t, e_out=e_out) * factor
        ...
        if delta != 0:
            factor = delta / (_bend_function(epsilon=epsilon, phi=phi_0,
                                             nd=nd, n_as=n_as, e_as=e_as,
                                             e_f=e_f, t=t, e_out=e_out) - _bend_function(epsilon=epsilon,
                                                                                         phi=phi_0 - delta, nd=nd,
                                                                                         n_as=n_as, e_as=e_as, e_f=e_f,
                                                                                         t=t, e_out=e_out))
