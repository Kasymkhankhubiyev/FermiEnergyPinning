...
# перекинуть в Draw Graphs - сначала дергаем в MainWindow, дергаем в DrawGraphs
def get_calculated_values(E_g, epsilon, m_h, m_e, E_d, N_d0, E_as, N_as, T, E_out):
    args = {
...
def output_info(results, window):

    if results['message'] != 'ok':
        messagebox.showerror(title='error', message=results['message'])
    else:
        #где-то выводять результат текстом
        messagebox.showinfo(title='solution',
                 message=f"fermi level:\t\t{results['E_f_s'][0]:.6f} [eV]\nbending of zone PHI:\t\t{results['phi']:.6f}"
